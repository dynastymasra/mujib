...
func FindAll(service service.ArticleServicer) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		...
		articles, err := service.FindAllArticle(r.Context())
		if err != nil {
			log.WithError(err).Errorln("Error get article by id from db")
		...
		statusCode := http.StatusOK
		w.WriteHeader(statusCode)
		fmt.Fprint(w, formatter.SuccessResponse(statusCode, articles).Stringify())
	}
}
